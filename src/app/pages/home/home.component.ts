...
import { objectToClass } from '../../shared/functions/parser.functions';
...
export class HomeComponent implements OnDestroy, OnInit {
    ...
    public ngOnInit(): void {
        ...
        this.userService.get(this.authUser?.uid ?? 'undefined').pipe(first()).subscribe({
            next: async (user: User | undefined) => {
                this.user = user;
                this.logHelper.logDefault('userObservable', 'current user', { value: user });
                                        ...
                                        if (connInfo.isReady()) {
                                            if (this.user !== undefined && connInfo.device !== undefined) {
                                                try {
                                                    ...
                                                    const devices = this.user?.devices?.map((dev: Device) => device.macAddress === dev.macAddress ? objectToClass<Device>(device, Device) : dev);
                                                    await this.userService.updateField({ id: this.user.id }, 'devices', devices, false);
            ...
            error: async (error: unknown) => {
                if (error instanceof Error) {
                    await this.messageService.displayErrorMessage(new LogInfo(HomeComponent.name, 'getUser', error), 'toast', true);
                }
            }
        });
    }
